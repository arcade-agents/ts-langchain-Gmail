...
export function setShims(shims, options = { auto: false }) {
    if (auto) {
        throw new Error(`you must \`import '@arcadeai/arcadejs/shims/${shims.kind}'\` before importing anything else from @arcadeai/arcadejs`);
    }
    if (kind) {
        throw new Error(`can't \`import '@arcadeai/arcadejs/shims/${shims.kind}'\` after \`import '@arcadeai/arcadejs/shims/${kind}'\``);
    }
    auto = options.auto;
    kind = shims.kind;
    fetch = shims.fetch;
